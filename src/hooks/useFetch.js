...
export const useFetch = (setRecipes) => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      await axiosInstance
        .get(
          `recipes/complexSearch?apiKey=${process.env.NEXT_PUBLIC_SPOONACULAR_APIKEY}&fillIngredients=true&query=${searchRecipe}&cuisine=${filterCuisine}&diet=${filterDiet}&intolerances=${filterIntolerance}&includeIngredients=${myFridgeIngredients}&number=3&addRecipeInformation=true`
        )
