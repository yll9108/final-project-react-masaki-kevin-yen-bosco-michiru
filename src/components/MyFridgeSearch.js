...
const MyFridgeSearch = () => {
  ...
  const { autoComplete, setAutoComplete } = useAutoCompleteFetch(preventSearchRef, searchKeyword);
  ...
  return (
    <div>
      <InputBar type="text" value={input} onChange={handleInputChange} id="inputIngredients" placeholder="Search" autoFocus></InputBar>

      {input && autoComplete.length > 0 && (
        <AutoCompleteDropdown>
          {autoComplete.map((item, index) => (
            <AutoCompleteBtn onClick={() => handleAutoCompleteClick(item)} key={index}>
              {item}
            </AutoCompleteBtn>
          ))}
        </AutoCompleteDropdown>
